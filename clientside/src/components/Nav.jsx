        ...
        <div className="flex justify-between items-center h-16 sm:h-18 md:h-20">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4">
            {isAuthenticated() ? (
                ...
                {isDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-48 sm:w-56 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50">
                    ...
                    <button
                      ...
                      className="flex items-center space-x-2 sm:space-x-3 w-full px-3 sm:px-4 py-2 text-left text-gray-700 hover:bg-gray-50"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center space-x-2 sm:space-x-3 w-full px-3 sm:px-4 py-2 text-left text-red-600 hover:bg-red-50"
                    >
              ...
              <button
                onClick={handleLogin}
                className="text-cyan-500 hover:text-cyan-600 font-medium text-xs sm:text-sm border border-cyan-400 px-2 sm:px-3 md:px-4 py-1 rounded-lg transition-colors"
              >
                Login
              </button>
            )}
            ...
            <button
              onClick={handleCart}
              className="relative flex gap-1 sm:gap-2 items-center justify-center w-12 sm:w-14 md:w-16 h-7 sm:h-8 border border-cyan-400 rounded-lg hover:bg-cyan-50 transition-colors"
            >
              {/* Cart Icon */}
              <ShoppingCart className="w-3 h-3 sm:w-4 sm:h-4 text-cyan-500 hover:text-gray-800" />

              {/* Cart Count */}
              <span className="text-xs sm:text-sm text-cyan-600 font-semibold">
                {isAuthenticated() ? cartCount : 0}
              </span>

              {/* Notification Badge */}
              {cartCount > 0 && (
                <span className="absolute -top-1 sm:-top-2 -right-1 sm:-right-2 flex items-center justify-center w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-xs font-bold text-white bg-red-500 rounded-full">
